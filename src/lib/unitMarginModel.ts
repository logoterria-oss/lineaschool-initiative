 ...
 *
 * ПРОПУСКИ И СПИСАНИЯ. Считаем не по присутствию, а по деньгам. Пропуск без
 * уважительной причины CRM списывает — место оплачено и в выручку входит,
 * хотя ребёнка не было. Уважительный пропуск и отработка списания не дают:
 * место занято, педагогу заплатили, а денег нет. Поэтому наполняемость в
 * расчёте — ОПЛАЧЕННАЯ (списаний ÷ занятий), а не число пришедших.
 *
 * Средние цены берём из факта CRM: сумма списаний за месяц ÷ число
 * ОПЛАЧЕННЫХ мест. Никаких прайсов и допущений — сколько реально
 * списали, столько и заработали.
 *
...
export function calcUnit(
  ...
  // ОПЛАЧЕННАЯ наполняемость: сколько мест на занятии принесли деньги.
  // У индивидуальных обычно 1, но бывает меньше — занятие провели, а
  // списания не было (отработка, уважительный пропуск): расход есть,
  // выручки нет. Прогул со списанием, наоборот, место оплачивает.
  const size = Math.max(0, safe(side.groupSize));

  // ВЫРУЧКА ЗАНЯТИЯ — сумма списаний со всех оплаченных мест.
  const revenue = pricePerClient * size;
  const revenueFormula =
    form === 'group'
      ? `${fmtMoney2(pricePerClient)} × ${round2(size)} опл. мест = ${fmtMoney2(revenue)}`
      : size === 1
        ? `${fmtMoney2(pricePerClient)} за занятие`
        : `${fmtMoney2(pricePerClient)} × ${round2(size)} опл. = ${fmtMoney2(revenue)}`;
