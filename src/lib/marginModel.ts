 ...
 *
 * 2. ПРЯМЫЕ (переменные) расходы — то, что тратится на конкретного ребёнка
 *    и меняется вместе с объёмом занятий:
 ...
 *
 * 3. КОСВЕННЫЕ (постоянные, накладные) расходы — школа целиком: зарплаты
 *    не-педагогов, реклама, ПО, бухгалтер. Они не зависят от объёма продаж,
 *    поэтому в МАРЖИНАЛЬНОСТЬ не входят. На один абонемент они ложатся
 *    долей: доля = уроки этого абонемента / все ученико-уроки школы за месяц.
 ...
 * ВАЖНО про термины
 * -----------------
 * МАРЖИНАЛЬНОСТЬ = (Выручка − Прямые расходы) / Выручка × 100%. Только
 * переменные затраты, никакой аренды, управленцев и рекламы. Эти деньги как
 * раз и идут на покрытие постоянных расходов и формирование прибыли.
 * Показатель «после косвенных и налога» — это уже не маржинальность,
 * а РЕНТАБЕЛЬНОСТЬ ПО ЧИСТОЙ ПРИБЫЛИ, и называем её в интерфейсе именно так.
 */
...
export interface MarginResult {
  ...
  grossProfit: number;
  /** Маржинальность, %: доля маржи в выручке. Постоянные расходы не в счёт. */
  grossMarginPercent: number;
  /** Чистая прибыль: маржа − косвенные расходы − налог. */
  netProfit: number;
  /** Рентабельность по чистой прибыли, %. Это НЕ маржинальность. */
