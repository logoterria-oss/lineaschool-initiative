...
export function buildHomeworkReport(marks: HwMark[], examDate?: string): string {
  ...
  if (recent.length > 0 && earlier.length > 0) {
    ...
    // Данных на сравнение пока не хватает — честно об этом говорим
    parts.push(
      'Все отметки относятся к последним трём месяцам, поэтому сравнение с более ранним ' +
        'периодом пока недоступно — динамика будет видна в следующих диагностиках.',
    );
  }

  // --- 3. Рекомендация по итогам ---
  if (share < 50) {
    parts.push(
      'Рекомендуется наладить регулярность выполнения домашних заданий: без ежедневной ' +
        'самостоятельной работы закрепление материала занятий существенно замедляется.',
    );
  } else if (all.green > 0 && excellentShare(all) < 50 && share >= 50) {
    parts.push(
      'Рекомендуется уделить внимание качеству выполнения заданий: важна не только ' +
        'регулярность, но и тщательность проработки каждого упражнения.',
    );
  }
