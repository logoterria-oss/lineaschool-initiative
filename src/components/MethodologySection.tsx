  ...
  const BrainSVG = ({ activeAreas, className = "" }: { activeAreas: string[], className?: string }) => (
    <div className={`relative w-full h-full ${className}`}>
      <img 
        ...
        style={{
          filter: activeAreas.length > 0 ? 'brightness(1.1) contrast(1.2)' : 'brightness(0.9)',
          transition: 'filter 0.3s ease'
        }}
      />
      {/* Невидимые области для интерактивности */}
      <div className="absolute inset-0 pointer-events-none">
        {/* Лобная доля - розовая область слева */}
        <div 
          className={`absolute transition-all duration-300 ${
            activeAreas.includes('frontal') ? 'bg-blue-500 bg-opacity-30' : ''
          }`}
          style={{
            left: '5%',
            top: '20%',
        ...
        {/* Теменная доля - оранжевая область сверху */}
        <div 
          className={`absolute transition-all duration-300 ${
            activeAreas.includes('parietal') ? 'bg-orange-500 bg-opacity-30' : ''
          }`}
          style={{
            left: '40%',
            top: '10%',
        ...
        {/* Височная доля - зеленая область снизу */}
        <div 
          className={`absolute transition-all duration-300 ${
            activeAreas.includes('temporal') ? 'bg-pink-500 bg-opacity-30' : ''
          }`}
          style={{
            left: '15%',
            top: '50%',
            width: '40%',
            height: '35%',
            borderRadius: '30% 40% 50% 30%',
          }}
        />
        
        {/* Затылочная доля - красная область справа */}
        <div 
          className={`absolute transition-all duration-300 ${
            activeAreas.includes('occipital') ? 'bg-green-500 bg-opacity-30' : ''
          }`}
          style={{
            left: '70%',
            top: '30%',
        ...
        {/* Моторная кора - синяя область сверху в центре */}
        <div 
          className={`absolute transition-all duration-300 ${
            activeAreas.includes('motor') ? 'bg-purple-500 bg-opacity-30' : ''
          }`}
          style={{
            left: '35%',
            top: '5%',
