...
const API_URL = 'https://functions.poehali.dev/65751635-528e-4830-bc09-e0b9c5344580';
...
const QuestionnairesView = () => {
  ...
  const viewDetails = (id: number) => setSelectedResponse(selectedResponse === id ? null : id);
  const formatDate = (dateStr: string) => new Date(dateStr).toLocaleDateString('ru-RU');

  if (isLoading) {
    return (
      <div className="text-center py-12">
        <Icon name="Loader2" className="animate-spin h-8 w-8 mx-auto mb-2 text-green-600" />
              ...
              <div className="p-6">
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    <h3 className="text-lg font-semibold text-gray-900 mb-2">{response.child_name}</h3>
                  ...
                  <div className="flex gap-2 ml-4">
                    <Button onClick={() => viewDetails(response.id)} variant="outline" size="sm">
...
function DetailedView({ responseId }: { responseId: number }) {
  ...
  useEffect(() => {
    (async () => {
      setIsLoading(true);
      try {
        const response = await fetch(`${API_URL}?id=${responseId}`);
  ...
  if (isLoading) return <div className="text-center py-4 text-gray-600">Загрузка...</div>;
  if (!details) return <div className="text-center py-4 text-gray-600">Данные не найдены</div>;
