...
const RecurringPaymentsView = () => {
  const { items, loading, error, saving, save, markPaid, remove } = useRecurringPayments();
    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">Постоянные траты с датами и категориями</p>
        </div>
        <button
          onClick={openNew}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 sm:col-span-2">
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            {byCategory.map((c) => (
              ...
              >
                ...
                {c.category}: {rub(Math.round(c.monthly))}
              </span>
            ))}
          </div>
        </div>
      ...
      <HintBox
        ...
        hints={[
          'Добавляйте каждую постоянную трату: назначение, категорию, сумму и дату следующего платежа.',
          'Периодичность: ежемесячно, раз в квартал/полгода/год или свой интервал в месяцах.',
          'После оплаты нажмите «Оплачено» — дата автоматически сдвинется на следующий период.',
          'Платежи, до которых осталось 7 дней и меньше, подсвечиваются; просроченные — красным.',
        ]}
      />
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        <button
          ...
          className={`px-3 py-1.5 rounded-lg text-sm font-medium border transition-colors ${catFilter === 'all' ? 'bg-gray-800 text-white border-gray-800' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`}
        >
          Все ({items.length})
        </button>
        {CATEGORIES.map((c) => {
          ...
          return (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-sm font-medium border transition-colors ${catFilter === c ? 'bg-gray-800 text-white border-gray-800' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`}
            >
              {c} ({count})
            </button>
          );
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-dashed border-gray-300 p-10 text-center">
          ...
          <h3 className="text-lg font-semibold text-gray-700 mb-1">Пока нет платежей</h3>
          <p className="text-gray-400 text-sm">Нажмите «Добавить платёж», чтобы завести первую регулярную трату.</p>
        </div>
      ) : (
        <div className="space-y-2.5">
          {filtered.map((p) => {
            ...
            return (
              <div
                key={p.id}
                className={`bg-white rounded-xl border shadow-sm p-4 flex flex-col md:flex-row md:items-center gap-3 ${overdue ? 'border-red-300 bg-red-50/40' : soon ? 'border-amber-300 bg-amber-50/40' : 'border-gray-200'}`}
              >
                ...
                <div className="text-right md:w-40 flex-shrink-0">
                  ...
                  <div className={`text-xs font-medium mt-0.5 ${overdue ? 'text-red-600' : soon ? 'text-amber-600' : 'text-gray-500'}`}>
                    {fmtDate(p.next_date)}
                  </div>
                  <div className={`text-[11px] mt-0.5 ${overdue ? 'text-red-600' : soon ? 'text-amber-600' : 'text-gray-400'}`}>
                    {overdue ? `Просрочен на ${Math.abs(dl)} дн.` : dl === 0 ? 'Сегодня' : `Через ${dl} дн.`}
                  </div>
                </div>
