...
# Сертификаты Минцифры (Russian Trusted CA). Т-Банк переходит на российский
# доверенный УЦ, поэтому серверный запрос Init должен доверять этим корням,
# иначе соединение упадёт с ошибкой проверки TLS-сертификата.
# https://developer.tbank.ru/eacq/intro/certificates/migration-russian-trusted-ca
#
# Корневой и промежуточный сертификаты вшиты прямо в код — так оплата не
# зависит от доступности gu-st.ru в момент запроса. Свежие версии всё равно
# пробуем подтянуть, но при недоступности используем встроенные.
...
def _build_ssl_context() -> ssl.SSLContext:
    ...
    Сначала в контекст добавляются вшитые сертификаты Russian Trusted CA
    (гарантированный fallback), затем пробуем подтянуть свежие версии с
    серверов Госуслуг. Если сеть недоступна — работаем на встроенных.
    """
    ...
    try:
        ctx.load_verify_locations(cadata=_RUSSIAN_TRUSTED_CA_PEM)
        print('Russian Trusted CA loaded from bundled PEM')
    except Exception as e:
        print(f'Bundled CA load failed: {e}')
    ...
    for url in _RUSSIAN_TRUSTED_CA_URLS:
        try:
            ...
            print(f'Russian Trusted CA refreshed from {url}')
        except Exception as e:
