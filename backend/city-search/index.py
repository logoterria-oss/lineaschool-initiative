...
REGION_TIMEZONES = {
    ...
    'нижегородская': 0, 'казань': 0, 'татарстан': 0,
    'волгоградская': 0, 'калмыкия': 0,
    ...
    # МСК+1 (UTC+4) — Самара, Ижевск, Ульяновск, Саратов, Астрахань
    'самарская': 1, 'удмуртская': 1, 'ульяновская': 1,
    'саратовская': 1, 'астраханская': 1,

    # МСК+2 (UTC+5) — Екатеринбург, Челябинск, Тюмень, Курган, Уфа, Пермь, ХМАО, ЯНАО
    'свердловская': 2, 'челябинская': 2, 'тюменская': 2, 'курганская': 2,
    'ханты-мансийский': 2, 'ямало-ненецкий': 2, 'оренбургская': 2,
    'башкортостан': 2, 'пермский': 2,

    # МСК+3 (UTC+6) — Омск
    'омская': 3,

    # МСК+4 (UTC+7) — Новосибирск, Томск, Кемерово, Барнаул, Красноярск, Тыва, Хакасия
    'новосибирская': 4, 'томская': 4, 'кемеровская': 4, 'алтайский': 4,
    'алтай': 4, 'красноярский': 4, 'тыва': 4, 'хакасия': 4,

    # МСК+5 (UTC+8) — Иркутск, Бурятия
    'иркутская': 5, 'бурятия': 5,

    # МСК+6 (UTC+9) — Чита, Якутск, Благовещенск
    'забайкальский': 6, 'якутия': 6, 'саха': 6, 'амурская': 6,

    # МСК+7 (UTC+10) — Хабаровск, Владивосток, ЕАО
    'хабаровский': 7, 'приморский': 7, 'еврейская': 7,
...
def get_timezone_by_region(region: str) -> int | None:
    """Часовой пояс по названию региона.

    Сравниваем по границам слов, а не по подстроке: «Томская обл» содержит
    в себе «омская», и простой поиск подстроки отправлял Томск в часовой
    пояс Омска. Ключи из нескольких слов («марий эл», «северная осетия»)
    проверяем целиком.
    """
    region_lower = region.lower().replace('ё', 'е')
    # Длинные ключи проверяем первыми: «ямало-ненецкий» должен победить
    # более короткий «ненецкий» (это разные округа и разные пояса).
    for key, offset in sorted(REGION_TIMEZONES.items(), key=lambda kv: -len(kv[0])):
        if re.search(rf'(?<![а-яa-z]){re.escape(key)}(?![а-яa-z])', region_lower):
            return offset
    return None
